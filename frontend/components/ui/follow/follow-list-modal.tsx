  ...
  const fetchUsers = async () => {
    try {
      ...
      const usersWithFollowStatus = await Promise.all(
          data.map(async (user) => {
            // isFollowing: Biz onları takip ediyor muyuz?
            const isFollowing = await followService.isFollowing(user.id.toString());

            // isFollowedBy: Onlar bizi takip ediyor mu?
            const isFollowedBy = type === "followers" ? true : await followService.isFollowing(currentUserId.toString(), user.id.toString());
    ...
    if (user.isFollowing) {
      return (
          ...
          <span className="group-hover:hidden flex items-center">
            ...
            {user.isFollowing && user.isFollowedBy ? 'Karşılıklı Takip' : 'Takip Ediliyor'}
          </span>
      ...
      return (
          ...
          >
            ...
            <span>{user.isFollowedBy ? 'Geri Takip Et' : 'Takip Et'}</span>
          </Button>
      )
    }
  }
